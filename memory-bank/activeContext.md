...
## Current Task
- Harden desktop release workflow by pinning GitHub Actions to immutable commit SHAs.

## Pending
- Push/tag repository so GitHub Actions can publish `.exe`, `.zip`, `.app`, `.dmg` to Releases and validate pinned action execution.
...
- Workflow now passes explicit macOS bundle args `--bundles app,dmg --target universal-apple-darwin` on macos-latest.
